...
React Native의 컴포넌트는 [JavaScript를 사용하여 스타일이 지정됩니다](style). 색상 props는 일반적으로 [웹에서 CSS가 동작하는 방식](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value)과 일치합니다. 각 플랫폼에서의 색상 사용에 관한 일반적인 가이드는 아래에서 확인할 수 있습니다.
...
React Native에는 플랫폼의 디자인과 사용자 기본 설정을 최대한 활용할 수 있도록 설계된 여러 Color API가 있습니다.

- [PlatformColor](platformcolor)를 사용하면 플랫폼의 색상 시스템을 참조할 수 있습니다.
- [DynamicColorIOS](dynamiccolorios)는 iOS 전용이며, 라이트 모드 또는 다크 모드에서 사용할 색상을 지정할 수 있습니다.

## 색상 표현 방식

### Red Green Blue (RGB)

React Native는 16진수 표기법과 함수형 표기법 모두에서 `rgb()`와 `rgba()`를 지원합니다.
...
React Native는 함수형 표기법으로 `hsl()`과 `hsla()`를 지원합니다.
...
React Native는 함수형 표기법으로 `hwb()`를 지원합니다.
...
React Native는 `int` 값으로도 색상을 지원합니다(RGB 색상 모드):

- `0xff00ff00` (0xrrggbbaa)

:::caution
이것은 Android [Color](https://developer.android.com/reference/android/graphics/Color) 정수값 표현과 유사해 보일 수 있지만, Android에서 값은 SRGB 색상 모드(0xaarrggbb)로 저장됩니다.
:::

### 이름이 있는 색상

React Native에서는 색상 이름 문자열을 값으로 사용할 수도 있습니다.

:::info
React Native는 소문자 색상 이름만 지원합니다. 대문자 색상 이름은 지원되지 않습니다.
:::

#### `transparent`

이것은 `rgba(0,0,0,0)`의 단축 표현으로, [CSS3](https://www.w3.org/TR/css-color-3/#transparent)와 동일합니다.

#### 색상 키워드

이름이 있는 색상 구현은 [CSS3/SVG 명세](https://www.w3.org/TR/css-color-3/#svg-color)를 따릅니다.
